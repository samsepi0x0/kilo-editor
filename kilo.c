...
void die(const char *s) {
  /* perror comes from stdio.h, exit comes from stdlib.h.
   * perror looks at global errno variable and prints a descriptive error for it.*/
...
void editorDrawStatusBar(struct abuf *ab) {
  ...
  int len = snprintf(status, sizeof(status), "%.20s - %d lines %s", E.filename ? E.filename : "New file", E.numrows, E.dirty ? "(Modified)" : "");
